...
.get(function (req, res, next) {
    books.findOne({"name":req.params.bookName}, function (err, book) {
        if (err) throw err;
        res.json(book);
    });
})
...
.delete(function (req, res, next) {
    books.findOne({"name":req.params.bookName}, function (err, resp) {
        if (err) throw err;
        res.json(resp);
    }).remove().exec();
});
...
.delete(function (req, res, next) {
    books.findOne({"category":req.params.categoryName}, function (err, resp) {
        if (err) throw err;
        res.json(resp);
    }).remove().exec();
});
...
//Get price of a book
.get(function (req, res, next) {
    books.findOne({"name":req.params.bookName}, {"price":1}, function (err, book) {
        if (err) throw err;
        res.json(book);
    });
})
...
.post(function (req, res, next) {
    ...
    books.findOneAndUpdate({"name":req.params.bookName}, {"$set":{"price": req.body.price}}, function (err, resp){
        if (err) console.log(err);
        res.json(resp);        
    });
});
